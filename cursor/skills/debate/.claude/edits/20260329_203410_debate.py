...
NUM_AGENTS = 3
# Wall-clock cap for sequential open→challenge→resolve (not eval). 0 = disabled.
DEFAULT_DEBATE_DEADLINE_SEC = 600
...
def slot_for_turn(turn_idx: int, num_agents: int) -> int:
    """Map each sequential turn to exactly one debater slot (one CLI process per turn).

    open → slot 0 (Alex), challenge → 1 (Jordan), resolve → 2 (Sam). Never two
    speakers on the same turn index.
    """
    if num_agents >= 3:
        return turn_idx % 3
    return turn_idx % max(1, num_agents)


def format_prior_turns_for_prompt(prior_turns: list[dict], max_chars: int = 14000) -> str:
    """Rich transcript of earlier turns on this finding (for judgment context)."""
    if not prior_turns:
        return "(no prior turns yet for this finding.)"
    chunks: list[str] = []
    for t in prior_turns:
        chunks.append(
            f"### {t.get('agent_name', '?')} — role: {t.get('role', '?')}\n"
            f"- **vote:** {t.get('vote', '?')}\n"
            f"- **stance:** {t.get('stance', '')}\n"
            f"- **rationale:**\n{t.get('rationale', '')}\n"
        )
    text = "\n".join(chunks)
...
**Prior turns on this same finding** (read carefully; extend or challenge this judgment chain — do not ignore it):
{prior_block}
...
def load_debate_digest(work_dir: Path, max_chars: int = 20000) -> str:
    """Compact text from debate_turns.jsonl for improvement-plan grounding."""
    records = read_jsonl_tail(work_dir / "debate_turns.jsonl", limit=0)
    if not records:
        return ""
    lines_out: list[str] = []
    for rec in records:
        ...
        role = rec.get("role", "?")
        who = rec.get("agent_name", "?")
        parsed = rec.get("parsed") if isinstance(rec.get("parsed"), dict) else {}
        vote = str(parsed.get("vote", "")).lower() if parsed else ""
        stance = str(parsed.get("stance", ""))[:400]
        rat = str(parsed.get("rationale", ""))[:1200]
        lines_out.append(
            f"[{fid}] {role} — {who} | vote={vote} | stance: {stance}\n  rationale: {rat}\n"
        )
    text = "\n".join(lines_out)
    if len(text) > max_chars:
        return "...(truncated)\n" + text[-max_chars:]
...
def debate_status_agents_triplet(active_slot: int | None, active_status: str) -> list[dict]:
    """UI row: three fixed debaters; exactly one may be non-idle (the active agent)."""
    row: list[dict] = []
    for i in range(min(3, len(DEBATER_PERSONAS))):
        persona = persona_for_slot(i)
        if active_slot is not None and i == active_slot:
            st = active_status
        else:
            st = "idle"
        row.append(
...
# find_git_root is imported from _utils
...
def build_improvement_plan_prompt(persona: dict, draft_so_far: str, context: dict) -> str:
    ...
    digest = (context.get("debate_digest") or "").strip()
    digest_section = ""
    if digest:
        digest_section = (
            f"\n**Debate-turn digest (what was argued in sequential rounds):**\n```\n{digest}\n```\n"
        )
    return rf"""You are **{persona["display_name"]}** — {persona["title"]}.
**Intent:** {persona["intent"]}
**Your planning lens:** {persona["plan_brief"]}

Context from the debate (may be partial if a time limit stopped some rounds):
...
def phase_improvement_plan(
    ...
    rating: str,
    *,
    time_limited_debate: bool = False,
) -> None:
    print("\n=== OPTIONAL: Improvement plan (3 persona passes) ===")
    plan_path = work_dir / "improvement-plan.md"
    if time_limited_debate:
        header = (
            "# Improvement plan\n\n"
            "_Synthesized after a **time-limited** debate (not every finding "
            "completed all three rounds). Each role appended their lane; edit as a team._\n\n"
            f"- Target: `{target}`\n"
    ...
    ctx = {
        ...
        "debate_digest": load_debate_digest(work_dir),
    }
...
def phase_vote(
    ...
    if use_file:
        ...
        safe_write_json(findings_file, vote_findings)
        vote_payload = (
...
def phase_sequential_findings_debate(
    ...
    if debate_deadline_monotonic is not None:
        ...
        dl_note = f" Wall-clock cap: {remain:.0f}s remaining for new debate turns."
    ...
    write_status(
        status_file,
        {
            ...
            "debate_track": debate_track,
            "debate_deadline_hit": deadline_hit,
            "agents": debate_status_agents_triplet(None, "idle"),
        },
    )

    sub = (
        f"Debated {len(to_debate)} finding(s) (deadline stopped early)."
        if deadline_hit
        else f"Debated {len(to_debate)} finding(s), {len(TURN_ROLES)} turns each"
    )
    append_event(
        work_dir,
        {
            ...
            "title": "Sequential rounds ended" if deadline_hit else "Sequential rounds complete",
            "subtitle": sub,
...
def execute_debate(
    ...
    *,
    ...
    debate_deadline_seconds: wall-clock cap from start of sequential debate (None/0 = off).
      When hit, no new turns start; tally runs; improvement plan runs if not already produced
      (including when improvement_plan was False).
    """
    get_agent_paths()
    ...
    if print_banner:
        ...
        if debate_mode == "sequential" and debate_deadline_seconds and debate_deadline_seconds > 0:
            print(f"  Debate deadline: {debate_deadline_seconds}s wall clock (sequential rounds)")
        print("=" * 60)
    ...
    results = {
        ...
        "use_git_worktrees": use_git_worktrees,
        "debate_deadline_seconds": debate_deadline_seconds,
        "debate_deadline_hit": debate_deadline_hit,
    ...
    plan_path = work_dir / "improvement-plan.md"
    if improvement_plan or debate_deadline_hit:
        phase_improvement_plan(
            ...
            score,
            rating,
            time_limited_debate=debate_deadline_hit,
        )
        results["improvement_plan_path"] = str(plan_path)
        if improvement_plan and debate_deadline_hit:
            results["improvement_plan_trigger"] = "requested_and_time_limited"
        elif debate_deadline_hit:
            results["improvement_plan_trigger"] = "deadline_fallback"
        else:
            results["improvement_plan_trigger"] = "requested"
        safe_write_json(results_file, results)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=NUM_AGENTS,
        help=f"Deprecated: debater count is fixed at {NUM_AGENTS} (Alex, Jordan, Sam).",
    )
    ...
    parser.add_argument(
        "--debate-deadline-seconds",
        type=int,
        default=DEFAULT_DEBATE_DEADLINE_SEC,
        metavar="SEC",
        help=(
            "Wall-clock cap for sequential open/challenge/resolve (0=off). "
            "When hit: stop new turns, tally, then write improvement-plan.md "
            "(even if --improvement-plan was not set)."
        ),
    )
    ...
    if args.agents != NUM_AGENTS:
        print(
            f"[debate] Ignoring --agents {args.agents}: using fixed trio ({NUM_AGENTS} debaters).",
            file=sys.stderr,
        )
    ...
    if args.hall_url:
        ...
        payload = json.dumps({
            ...
            "debate_deadline_seconds": max(0, args.debate_deadline_seconds),
        }).encode("utf-8")
    ...
    dl_sec = max(0, args.debate_deadline_seconds)
    code = execute_debate(
        target,
        work_dir,
        num_agents,
        ...
        improvement_plan=args.improvement_plan,
        use_git_worktrees=args.git_worktrees,
        debate_deadline_seconds=dl_sec if dl_sec > 0 else None,
    )
    sys.exit(code)
