---
name: swarm
description: When the user invokes /swarm, decompose the work into n parallel subagents (n >= 2), scaling n up with task complexity—prefer more agents for harder work. Use the Task tool to run isolated subagents and then synthesize results.
---
...
3. **How to run the swarm:** Use the **`Task`** tool to launch **separate subagents in parallel** (same turn if possible). Each subagent gets:
...
Use the **general-purpose** subagent type for swarm workers unless the user asks for the isolated worktree / best-of-n runner variant for experiments.
